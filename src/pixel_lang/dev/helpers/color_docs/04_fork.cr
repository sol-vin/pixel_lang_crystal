...
class Fork
  ...
  def self.make_color(direction_1, direction_2, direction_3 = nil, direction_4 = nil)
    d1_bits = Direction::DIRECTIONS.index(direction_1).as(Int32) << DIRECTION_1_BITSHIFT
    
    d2_bits = Direction::DIRECTIONS.index(direction_2).as(Int32) << DIRECTION_2_BITSHIFT
    
    d3_bits = 0
    unless direction_3.nil?
      d3_bits = Direction::DIRECTIONS.index(direction_3.as(Symbol)).as(Int32) << DIRECTION_3_BITSHIFT
      d3_bits += 1 << DIRECTION_3_BOOL_BITSHIFT
    end

    d4_bits = 0
    unless direction_4.nil?
      d4_bits = Direction::DIRECTIONS.index(direction_4.as(Symbol)).as(Int32) << DIRECTION_4_BITSHIFT
